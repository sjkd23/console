...
import { hasInternalRole, getMemberRoleIds, invalidateRoleCache } from '../lib/permissions.js';
...
export const setroles: SlashCommand = {
    data: new SlashCommandBuilder()
        .setName('setroles')
        .setDescription('Configure internal role mappings for this server (admin only)')
        ...
        try {
            ...
            if (!interaction.inGuild() || !interaction.guild) {
                await interaction.reply({
                    ...
                    // v14: ephemeral is the reliable way on defer/reply
                    flags: MessageFlags.Ephemeral, // ok for reply()
                });
                return;
            }

            // 2) ACK ASAP to avoid 3s timeout
            // Use ephemeral: true for v14 deferReply; flags may be ignored here.
            ...
            if (!hasAdminPerm) {
                await interaction.editReply('❌ You must have Discord Administrator permission to configure bot roles.');
                return;
            }
